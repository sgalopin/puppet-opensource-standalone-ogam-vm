...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "update", type: "shell", privileged: false, inline: <<-SHELL
	...
	git clone https://github.com/sgalopin/puppet-ogam /var/tmp/puppet-ogam
	...
	# Puppet apply
	# sudo -i puppet apply -e "class {'ogam': vhost_servername => 'agent.example.com', ... }"
	sudo -i puppet apply /etc/puppetlabs/code/environments/production/manifests/init.pp

	# Puppet tasks
	sudo /bin/bash /root/tmp/ogam/scripts/build_db.sh
	sudo /bin/bash /root/tmp/ogam/scripts/build_ogamserver.sh
	sudo /bin/bash /root/tmp/ogam/scripts/build_ogamdesktop.sh
	sudo /bin/bash /root/tmp/ogam/scripts/build_ogamservices.sh
  SHELL
